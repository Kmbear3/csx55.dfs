...
public class NotifySuccessor implements Event, Protocol{
    ...
    public NotifySuccessor(PeerEntry me, PeerEntry pred){
        this.me = me;
        this.pred = pred;
    }

    public NotifySuccessor(byte[] marshalledBytes){
        try {
            ByteArrayInputStream baInputStream =  new ByteArrayInputStream(marshalledBytes);
            DataInputStream din = new DataInputStream(new BufferedInputStream(baInputStream));

            if(din.readInt() != this.MESSAGE_TYPE){
                System.err.println("Type mismatch in Notifypredessor!!");
            }

            this.me = PeerEntry.unmarshallPeer(din);
            this.pred = PeerEntry.unmarshallPeer(din);
    ...
    public PeerEntry getLeavingPeerPred(){
        return this.pred;
    }
