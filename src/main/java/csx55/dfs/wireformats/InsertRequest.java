...
public class InsertRequest  implements Event, Protocol {

    // Message Type (int): REGISTER_REQUEST
    // IP address (String)
    // Port number (int)
    
    final int MESSAGE_TYPE = Protocol.INSERT_REQUEST;
    String IP;
    int port;
    int peerID;
    ...
    public InsertRequest(String IP, int port, int peerID){
        try {
            this.IP = IP;
            this.port = port;
            this.peerID = peerID;
    ...
    public InsertRequest(byte[] marshalledBytes) throws IOException{
